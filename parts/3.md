...
For three lines of *actual* code, those certainly couldn't be much different.
...
`p` is going to equal `list->first`, which is going to equal `entry`, but is not comparing `p` to
`entry`, it's comparing `p->next`.
...
And right now we are only updating `p->next`, which is inside a `struct node *`. Where are we updating the
...
If we want to delete the first element `list->first == entry`, therefore `((struct llist_node
*)list)->next == entry`. And if we are updating `p->next` only when `p->next == entry`, then we are
updating `p->next` in this case.
